...
from .utils.lora import fuse_lora_to_model


def _load_checkpoint_state_dict(checkpoint_path: str, map_location: str = "cpu"):
    if checkpoint_path.endswith(".safetensors"):
        try:
            ...
            raise ImportError(
                ...
                "Please install safetensors in the runtime environment."
            ) from exc
        state_dict = {}
...
class WanTI2V:
    ...
    def __init__(
        ...
        if pt_dir is not None:
            ...
            state_dict = _load_checkpoint_state_dict(pt_dir, map_location="cpu")
    ...
    def load_lora(self, lora_path, alpha=1.0):
        if lora_path is None:
            return 0
        updated = fuse_lora_to_model(self.model, lora_path, alpha=alpha, device="cpu", dtype=self.param_dtype)
        logging.info(f"LoRA fused into WanTI2V: {updated} tensors updated")
        return updated
    ...
    def t2v(self,
        ...
        print(f"[WanTI2V] input_prompt: {input_prompt}")
        print(f"[WanTI2V] n_prompt: {n_prompt}")
    ...
    def i2v(self,
        ...
        print(f"[WanTI2V-i2v] input_prompt: {input_prompt}")
        print(f"[WanTI2V-i2v] n_prompt: {n_prompt}")
        ...
        noise = torch.randn(
            ...
            generator=seed_g,
            device=self.device)
        #print("noise", noise.shape) # 48, 16, 46, 40
        # preprocess
        if not self.t5_cpu:
            self.text_encoder.model.to(self.device)
            context = self.text_encoder([input_prompt], self.device)
            ...
            context = [t.to(self.device) for t in context]
            context_null = [t.to(self.device) for t in context_null]
        #print("img", img.shape) #3, 1, 736, 640
        z = self.vae.encode([img])
        #print("z", len(z), z[0].shape) #48, 1, 46, 40
