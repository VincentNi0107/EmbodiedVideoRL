...
# ──────────────────────────────────────────────────────────────────────────────
# Evaluate vidar model on put_object_cabinet (SAPIEN simulation)
# ──────────────────────────────────────────────────────────────────────────────
#
# Usage (on a GPU node, e.g. qgpu2016):
#   cd /gpfs/projects/p33175/EmbodiedVideoRL
#   bash scripts/eval/eval_vidar_put_object_cabinet.sh
#
# Or via ssh:
#   ssh qgpu2016 "conda run -n wanx --no-capture-output \
#       --cwd /gpfs/projects/p33175/EmbodiedVideoRL \
#       bash scripts/eval/eval_vidar_put_object_cabinet.sh"
...
echo "[INFO] Starting video generation server on port ${PORT}..."
export PORT=${PORT}
...
echo "[INFO] Running evaluation: ${TASK_NAME} (${TASK_CONFIG})..."
cd vidar-robotwin
