"""Batch validation of BlockSizeRankingRewardScorer on rollout videos.

Processes all robotwin_blocks_ranking_size_* scene directories under --input-root.
Saves annotated videos tagged _CLEAN / _FAIL and a grand summary JSON.

Usage:
    python validate_block_size_reward.py \
        --input-root data/outputs/rollout_robotwin_121 \
        --out-root   data/outputs/block_size_validation
"""
...
def main():
    parser = argparse.ArgumentParser(description="Batch block size ranking reward validation")
    ...
    parser.add_argument("--out-root", type=str,
                        default="data/outputs/block_size_validation",
                        help="Output directory for annotated videos and summaries")
    parser.add_argument("--pattern", type=str,
                        default="robotwin_blocks_ranking_size_*",
    ...
    parser.add_argument("--min-visible-frac", type=float, default=0.50)
    parser.add_argument("--ordering-margin", type=float, default=0.03)
    ...
    scorer = BlockSizeRankingRewardScorer(
        prompt=args.prompt,
    ...
    for scene_dir in scene_dirs:
        ...
        for vpath in videos:
            ...
            scene_results.append({
                ...
                "ordering_detail": result.get("ordering_detail", ""),
                "mean_cx": result.get("mean_cx", -1),
                "response": resp,
            })
