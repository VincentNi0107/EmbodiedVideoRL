...
class Robot:
    ...
    @staticmethod
    def _resolve_curobo_yml(yml_path):
        """Resolve ${VIDAR_ROOT} placeholder in curobo YAML to absolute path.

        Curobo resolves relative paths against its own asset directory, not the
        YAML file location. We use ${VIDAR_ROOT} as a portable placeholder and
        expand it here so the YAML contains absolute paths at load time.
        """
        import tempfile
        with open(yml_path, "r") as f:
            content = f.read()
        if "${VIDAR_ROOT}" not in content:
            return yml_path  # no placeholder, use original
        content = content.replace("${VIDAR_ROOT}", CONFIGS.ROOT_PATH.rstrip("/"))
        tmp = tempfile.NamedTemporaryFile(
            mode="w", suffix=".yml", dir=os.path.dirname(yml_path), delete=False
        )
        tmp.write(content)
        tmp.close()
        return tmp.name
    ...
    def set_planner(self, scene=None):
        ...
        # Resolve ${VIDAR_ROOT} placeholders for portability
        abs_left_curobo_yml_path = self._resolve_curobo_yml(abs_left_curobo_yml_path)
        abs_right_curobo_yml_path = self._resolve_curobo_yml(abs_right_curobo_yml_path)
